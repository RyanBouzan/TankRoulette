...
namespace TankRoulette
{
    public class BootstrapNetworkManager : NetworkBehaviour
    {
        ...
        public List<GameObject> _tankList = new();
        [SyncVar]
        ...
        private void SpawnPlayer(NetworkConnection conn, BootstrapNetworkManager BNM)
        {
          ...
          //  Debug.LogWarning("Calling SpawnPlayer TARGET RPC");
            ...
            SpawnComponents(conn, BNM, _tankList[MainMenuManager.GetSelectedTank()], MainMenuManager.GetPersona());
        }
        ...
        /// <param name="tank">Tank Prefab</param>
        /// <param name="parent">Deprecated</param>
        [ServerRpc(RequireOwnership = false)]
        private void SpawnComponents(NetworkConnection conn, BootstrapNetworkManager BNM, GameObject tank, string name)
        {
          //  Debug.LogWarning("Calling SpawnComponents SERVER RPC");
            //clone prefabs locally
            GameObject activeTank = Instantiate(tank, TestSpawn.position + new Vector3(BNM.spawnIndex, 0, 0), TestSpawn.rotation);
            ...
            //spawn over network
            ServerManager.Spawn(activeTank, conn);
            ServerManager.Spawn(activeCam, conn);

            //set as parent over the network
            InitializeObserver(BNM, activeTank, name);
            ...
            //not gonna lie I was not aware of structs before taking CS420, it would probably be more efficient to wrap this in a struct but whatever
            PlayerObject newPlayer = new(conn.FirstObject.gameObject, activeTank, activeCam);
            ...
            InitializeComponents(conn, BNM, newPlayer.GetClient(), newPlayer.GetTank(), newPlayer.GetCam());
        ...
        private void InitializeObserver(BootstrapNetworkManager BNM, GameObject tank, string name)
        {
        ...
        /// <param name="activeTank">Reference to tank</param>
        /// <param name="activeCam">Reference to camera</param>
        [TargetRpc]
        private void InitializeComponents(NetworkConnection conn, BootstrapNetworkManager BNM, GameObject activeClient, GameObject activeTank, GameObject activeCam)
        {
